...
double AChessboard::GetChessPieceSize() const { return ChessPieceSize; }
...
AChessPiece* AChessboard::AddNewChessPiece(PieceType Type, PieceColor Color, FVector2D Position)
{
	...
	AChessPiece* ChessPiece = GetWorld()->SpawnActor<AChessPiece>(
		PieceClass,
		FVector(Position[1] + 0.5, Position[0] + 0.5, 0.001) * 100 * TileSize,
		FRotator::ZeroRotator
	);
...
{
	return FVector2D(
		FMath::Floor(RealPosition[1] / TileSize / 100),
		FMath::Floor(RealPosition[0] / TileSize / 100)
	);
}
...
FVector AChessboard::GridToRealPosition(FVector2D GridPosition, uint32 ZIndex)
{
	return FVector(
		...
		GridPosition[0] + 0.5,
		0.0
	) * TileSize * 100 + ZIndex * HeightDiff;
}
